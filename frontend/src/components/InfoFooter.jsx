...
const sections = [
  {
    ...
    links: ["Açık Rıza Metni", "Ticari İleti Açık Rıza Metni", "Çerez Aydınlatma Metni", "Üye Müşteri Aydınlatma Metni", "Mesafeli Satış Sözleşmesi"],
  },
  {
    title: "KURUMSAL",
    links: ["Hakkında", "Bize Ulaşın", "İnsan Kaynakları", "Müşteri Hizmetleri", "İptal İade"],
  },
  {
    title: "KURUMSAL",
    links: ["Hakkında", "Bize Ulaşın", "İnsan Kaynakları", "Müşteri Hizmetleri", "İptal İade"],
...
function AccordionSection({ title, links }) {
  ...
  return (
    <div className="info-footer-accordion">
      <button className="info-footer-accordion-btn" onClick={() => setOpen(!open)}>
        <span className="info-footer-col-title mb-0">{title}</span>
        <span>{open ? "−" : "+"}</span>
      </button>
      <div className={`info-footer-accordion-body ${open ? "open" : ""}`}>
        {links.map((link, i) => (
          <a key={i} href="#0" className="info-footer-link">{link}</a>
        ))}
      </div>
    </div>
  );
      ...
      <div className="container">
        <div className="row g-4">
          ...
          <div className="col-12 col-md-3 text-center text-md-start">
            ...
            <p className="info-footer-address mb-1">Yardıma mı ihtiyacınız var?</p>
            <p className="info-footer-phone">444 56 50</p>
            <p className="info-footer-address">
              Sarımeşe Mah. Bağdat Cad. No:316 Kartepe/Kocaeli<br />
              destek@listensi.com<br />
            ...
            <div key={i} className="col-12 col-md-2">
              <div className="d-none d-md-block">
                ...
                {section.links.map((link, j) => (
                  <a key={j} href="#0" className="info-footer-link">{link}</a>
                ))}
              </div>

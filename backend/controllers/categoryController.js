...
// Tüm kategoriler (ağaç yapısı olarak)
exports.getCategories = async (req, res) => {
    const all = await Category.find().lean();

    // root kategoriler (parent: null)
    const map = {};
...
exports.createCategory = async (req, res) => {
    ...
    if (parent) {
        ...
        if (!parentDoc) return res.status(404).json({ message: 'Parent kategori bulunamadı.' });
